...
public class KaptchaController extends BaseController {
     ...
     */
    @GetMapping("/token")
    public DataResultResponse<String> getToken() {
        return DataResultResponse.ok(kaptchaService.generateToken());
    }
     ...
     */
    @PostMapping("/valid")
    public DataResultResponse<String> valid(@RequestBody @Valid KaptchaValidParam param) {
        kaptchaService.valid(param.getToken(), param.getCode(), param.getMatchDimensionKey());
        return DataResultResponse.ok("验证成功");
    }
}